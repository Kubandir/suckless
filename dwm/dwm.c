...
static const Layout *last_layout = NULL;
void
fullscreen(const Arg *arg)
{
    if (selmon->showbar) {
        // Save the current layout before switching to fullscreen
        last_layout = selmon->lt[selmon->sellt];
        // Toggle client fullscreen state
        if (selmon->sel)
            setfullscreen(selmon->sel, 1);
    } else if (last_layout) {
        // Restore the client's normal state
        if (selmon->sel)
...
void drawbar(Monitor *m) {
  ...
  for (i = 0; i < LENGTH(tags); i++) {
    w = TEXTW(tags[i]);
    // Always use SchemeNorm for text (this will make text black)
    drw_setscheme(drw, scheme[SchemeNorm]);
    drw_text(drw, x, 0, w, bh, lrpad / 2, tags[i], urg & 1 << i);
    
    // Draw bottom line for selected tags with SchemeSel
    if (m->tagset[m->seltags] & 1 << i) {
      ...
      int rh = 2;
      int ry = 17;
      rw = MAX(rw, 1); // Ensure width is positive
      drw_rect(drw, rx, ry, rw, rh, 1, urg & 1 << i);
    }
    
    // Handle occupied tags
    if (occ & 1 << i) {  
      if (m->tagset[m->seltags] & 1 << i) {
        // Tag is both active and occupied - draw a filled square
        drw_setscheme(drw, scheme[SchemeNorm]);
        drw_rect(drw, x + boxs, boxs, boxw, boxw, 1, urg & 1 << i);
      } else {
        // Tag is occupied but not active - draw an unfilled square
        drw_setscheme(drw, scheme[SchemeNorm]);
        drw_rect(drw, x + boxs, boxs, boxw, boxw, 0, urg & 1 << i);
      }
    }
  ...
  if ((w = m->ww - tw - stw - x) > bh) {
    // Always use SchemeNorm for the background of the title area
    drw_setscheme(drw, scheme[SchemeNorm]);
    drw_rect(drw, x, 0, w, bh, 1, 1);

    if (m->sel) {
      // Set foreground color to full white (0xFFFFFF)
      drw->scheme[ColFg].pixel = 0xFFFFFF;
      // Keep background as SchemeNorm
      drw->scheme[ColBg] = scheme[SchemeNorm][ColBg];

      // Draw the text with white foreground
